...
Template.Profile1.events({
    'change .fileInput': function(event, template) {
      FS.Utility.eachFile(event, function(file) {
        ...
        fileObj.campaignId = sessionStorage.campaignName;
        if(Uploads.find({'original.name': fileObj.original.name,'campaignId': sessionStorage.campaignName, 'creatorId': Meteor.userId()}).fetch().length!==0){
          if(confirm("Ad already exists, do you want to replace it?")) {
            Uploads.remove(Uploads.find({'original.name': fileObj.original.name,'campaignId': sessionStorage.campaignName, 'creatorId': Meteor.userId()}).fetch()[0]._id);
