...
Router.route('/campaignSetup',{
 	action: function () {
 		if (this.ready())
 			this.render('campaignSetup');
 		else
 			this.render('Loading');
 	}
});
...
Router.route('/workflow',{
	waitOn: function () {
 		return [Meteor.subscribe('uploads'),Meteor.subscribe('nodes'),Meteor.subscribe('campaigns'),Meteor.subscribe('workflows')];
 	},
 	action: function () {
 		if (this.ready())
